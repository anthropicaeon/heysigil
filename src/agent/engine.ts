...
const TOOLS: Anthropic.Tool[] = [
  {
    name: "swap_tokens",
    description: "Exchange one token for another on a blockchain. Use this when the user wants to swap, trade, or exchange tokens. Always confirm the details with the user before calling this.",
    input_schema: {
      ...
      properties: {
        ...
        chain: { type: "string", description: "Chain to swap on (default: base)", enum: ["base", "ethereum", "polygon", "arbitrum", "optimism"] },
      },
  ...
  {
    name: "check_balance",
    description: "Check the user's wallet balance. Shows ETH and token balances. Use this when the user asks about their balance, funds, portfolio, or how much they have.",
    input_schema: {
      type: "object" as const,
      properties: {
        chain: { type: "string", description: "Chain to check (default: base)" },
        token: { type: "string", description: "Specific token to check (optional — omit for all)" },
      },
      required: [],
    },
  },
  {
    name: "get_price",
    description: "Get the current price of a token. Use when the user asks about a token's price, cost, or value.",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  {
    name: "verify_project",
    description: "Start verifying ownership of a project. Use when the user wants to verify, claim, or stamp their Sigil for a project. Pass the link exactly as the user provided it.",
    input_schema: {
      type: "object" as const,
      properties: {
        link: { type: "string", description: "Project URL or identifier (e.g. 'https://github.com/org/repo', 'mysite.dev', '@handle')" },
      },
      required: ["link"],
    },
  },
...
Always ask for dev links (GitHub repos, websites) if not provided.`,
    input_schema: {
      type: "object" as const,
      properties: {
        name: { type: "string", description: "Token name (optional — auto-generated if not provided)" },
        symbol: { type: "string", description: "Token ticker/symbol (optional — auto-generated)" },
        description: { type: "string", description: "Brief description of the project" },
        isSelfLaunch: {
          type: "boolean",
          description: "true if the user is launching their own project (fees go to their wallet), false if launching for someone else's project (fees go to escrow until dev verifies)",
        },
        devLinks: {
          type: "array",
          items: { type: "string" },
          description: "Developer links — GitHub repos, websites, Instagram handles, etc.",
        },
      },
  ...
  {
    name: "send_tokens",
    description: "Send tokens to an Ethereum address. Always confirm the recipient address and amount with the user before calling this.",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  {
    name: "claim_reward",
    description: "Claim pool rewards for a verified project. Use when the user wants to claim their earnings or rewards.",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  {
    name: "pool_status",
    description: "Check the pool reward status for a project. Use when asking about pool performance, rewards, or earnings.",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  {
    name: "create_wallet",
    description: "Create a new wallet for the user. Use when the user asks to create a wallet or when they need one for transactions.",
    input_schema: {
      type: "object" as const,
      properties: {},
      required: [],
  ...
  {
    name: "export_key",
    description: "Export the private key for the user's wallet. This is a sensitive 2-step process: first request, then confirm. Use when the user explicitly asks to export or backup their private key.",
    input_schema: {
      type: "object" as const,
      properties: {
        action: { type: "string", enum: ["request", "confirm"], description: "Step: 'request' to initiate, 'confirm' to complete the export" },
      },
      required: ["action"],
    },
  },
...
function mapToolParams(toolName: string, input: Record<string, unknown>): Record<string, unknown> {
  switch (toolName) {
    case "swap_tokens":
      return { fromToken: input.fromToken, toToken: input.toToken, amount: input.amount, chain: input.chain || "base" };
    case "check_balance":
    ...
    case "launch_token":
      return { name: input.name, symbol: input.symbol, description: input.description, devLinks: input.devLinks, isSelfLaunch: input.isSelfLaunch };
    case "send_tokens":
      return { token: input.token, amount: input.amount, toAddress: input.toAddress, chain: input.chain || "base" };
    case "claim_reward":
...
export async function processMessage(
  ...
  if (client) {
    ...
    const contextMessages: Anthropic.MessageParam[] = session.messages
      .slice(-20)
      .map((m) => ({
        role: m.role,
        content: m.content,
    ...
    while (iterations < MAX_ITERATIONS) {
      ...
      if (toolBlocks.length === 0) {
        assistantMessage = textBlocks.map((b) => b.type === "text" ? b.text : "").join("\n").trim();
        break;
      }
      ...
      const partialText = textBlocks.map((b) => b.type === "text" ? b.text : "").join("\n").trim();
