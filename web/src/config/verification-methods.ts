 ...
 *
 * Defines available verification methods and their properties.
 * Ordered by recommendation level for UX (Hick's Law).
 */

import type { Method } from "@/types/verification";

/**
 * All verification methods.
 * First 3 are "recommended" (shown prominently), rest collapsed under "More options".
 */
export const VERIFICATION_METHODS: Method[] = [
    {
        id: "github_oauth",
        name: "GitHub OAuth",
...
export const RECOMMENDED_METHODS = VERIFICATION_METHODS.filter((m) => m.recommended);

/** Additional methods collapsed by default */
export const OTHER_METHODS = VERIFICATION_METHODS.filter((m) => !m.recommended);

/** Step labels for verification flow progress */
export const VERIFICATION_STEP_LABELS = ["Method", "Details", "Verify", "Stamp"];
