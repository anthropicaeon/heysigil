...
import { Check, ChevronsUpDown, Infinity as InfinityIcon, X as XIcon } from "lucide-react";
...
function Section({ sec, selectedPlan }: { sec: FeatureSection; selectedPlan: number }) {
    return (
        <div className="border-dark-gray border-b last:border-b-0">
            <div className="border-b-dark-gray flex min-h-20 items-center border-b p-8">
                <h3 className="text-foreground max-w-3xs text-lg font-medium">{sec.category}</h3>
            </div>
...
function Cell({ val }: { val: Cell }) {
    if (val === "unlimited") return <InfinityIcon className="text-foreground size-5" />;
